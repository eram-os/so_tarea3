#include "supermercado.cpp"
#include <string>
#include <iostream>
#include <vector>
#include <queue>
#include <curses.h>

using namespace std;
//para imprimir
string productos[10]={
 "Arvejas",
 "Banana",
 "Tomate",
 "Ketchup",
 "Arroz",
 "Tallarines",
 "Mayonesa",
 "Manzana",
 "Te",
 "Cafe"
};

int tiempo_ms = 1000;

void config_tiempos();
void config_tiempos_valores();

void config_tiempos(){
    int yMax, xMax;
    getmaxyx(stdscr, yMax, xMax);

    WINDOW *menuwin = newwin(8, 50, 7, (xMax - 50) / 2);
    keypad(menuwin, true);

    string opciones[5] =
    {
        "Cajero 1",
        "Cajero 2",
        "Clientes Cajero 1",
        "Clientes Cajero 2",
        "Volver"
    };

    int highlight = 0;
    int choice;

    while(1)
    {
        werase(menuwin);
        box(menuwin,0,0);

        int ancho = getmaxx(menuwin);

        for(int i=0;i<5;i++)
        {
            int x = (ancho - opciones[i].length()) / 2;

            if(i == highlight)
                wattron(menuwin, A_REVERSE);

            mvwprintw(menuwin, i + 1, x, "%s", opciones[i].c_str());

            wattroff(menuwin, A_REVERSE);
        }

        wrefresh(menuwin);
        choice = wgetch(menuwin);

        switch(choice)
        {
            case KEY_UP:
                highlight = (highlight - 1 + 5) % 5;
                break;

            case KEY_DOWN:
                highlight = (highlight + 1) % 5;
                break;

            case 10:

                switch(highlight)
                {
                    case 0:
                        config_tiempos_valores();   // Tiempo Cajero 1
                        break;

                    case 1:
                        config_tiempos_valores();   // Tiempo Cajero 2
                        break;

                    case 2:
                        config_tiempos_valores();   // Tiempo Clientes Cajero 1
                        break;

                    case 3:
                        config_tiempos_valores();   // Tiempo Clientes Cajero 2
                        break;

                    case 4:
                        delwin(menuwin);
                        return;
                }

                break;
        }
    }
}

void config_tiempos_valores(){
    int yMax, xMax;
    getmaxyx(stdscr, yMax, xMax);

    WINDOW *menuwin = newwin(8, 50, 7, (xMax - 50) / 2);
    keypad(menuwin, true);

    int valor = tiempo_ms;
    int choice;

    while (1)
    {
        werase(menuwin);
        box(menuwin, 0, 0);

        string titulo = "Tiempo entre acciones";
		string tiempo = "<   " + to_string(valor) + " ms   >";
        string ayuda = "ENTER";

        mvwprintw(menuwin, 2, (50 - titulo.length()) / 2, "%s", titulo.c_str());
        mvwprintw(menuwin, 4, (50 - tiempo.length()) / 2, "%s", tiempo.c_str());
        mvwprintw(menuwin, 6, (50 - ayuda.length()) / 2, "%s", ayuda.c_str());

        wrefresh(menuwin);
        choice = wgetch(menuwin);

        switch(choice)
        {
            case KEY_LEFT:
                if(valor > 500)
                    valor -= 100;
                break;

            case KEY_RIGHT:
                if(valor < 5000)
                    valor += 100;
                break;

            case 10:
                tiempo_ms = valor;
                delwin(menuwin);
                return;
        }
    }
};

void ventana_inicio()
{
    clear();
    box(stdscr, 0, 0);

    int yMax, xMax;
    getmaxyx(stdscr, yMax, xMax);

    const int ANCHO_PRODUCTO = 14;
    const int ALTO_CAJERO = 5;
    const int PRODUCTOS = 5;

    // Ancho total del cajero
    const int ANCHO_CAJERO = PRODUCTOS * ANCHO_PRODUCTO + (PRODUCTOS + 1);

    int cajero1_x = 5;
    int cajero1_y = 4;

    int cajero2_x = 5;
    int cajero2_y = cajero1_y + ALTO_CAJERO + 6;

    //==========================
    // CAJERO 1
    //==========================

    mvprintw(cajero1_y - 1,
             cajero1_x + (ANCHO_CAJERO - 8) / 2,
             "Cajero 1");

    mvprintw(cajero1_y + 2,
             cajero1_x - 3,
             "S");

    mvprintw(cajero1_y + 2,
             cajero1_x + ANCHO_CAJERO + 2,
             "Cliente 1");

    WINDOW *cajero1 = newwin(ALTO_CAJERO,
                             ANCHO_CAJERO,
                             cajero1_y,
                             cajero1_x);

    box(cajero1,0,0);

    // Dibujar divisiones verticales
    for(int i=1;i<PRODUCTOS;i++)
    {
        int columna = i * (ANCHO_PRODUCTO + 1);

        mvwaddch(cajero1,0,columna,ACS_TTEE);

        for(int fila=1;fila<ALTO_CAJERO-1;fila++)
            mvwaddch(cajero1,fila,columna,ACS_VLINE);

        mvwaddch(cajero1,ALTO_CAJERO-1,columna,ACS_BTEE);
    }

    // Escribir productos
    for(int i=0;i<PRODUCTOS;i++)
    {
        int inicio = 1 + i * (ANCHO_PRODUCTO + 1);

        string nombre = productos[i];

        mvwprintw(cajero1,
                  2,
                  inicio + (ANCHO_PRODUCTO - nombre.length())/2,
                  "%s",
                  nombre.c_str());
    }

    wrefresh(cajero1);

    //==========================
    // CAJERO 2
    //==========================

    mvprintw(cajero2_y - 1,
             cajero2_x + (ANCHO_CAJERO - 8) / 2,
             "Cajero 2");

    mvprintw(cajero2_y + 2,
             cajero2_x - 3,
             "S");

    mvprintw(cajero2_y + 2,
             cajero2_x + ANCHO_CAJERO + 2,
             "Cliente 2");

    WINDOW *cajero2 = newwin(ALTO_CAJERO,
                             ANCHO_CAJERO,
                             cajero2_y,
                             cajero2_x);

    box(cajero2,0,0);

    for(int i=1;i<PRODUCTOS;i++)
    {
        int columna = i * (ANCHO_PRODUCTO + 1);

        mvwaddch(cajero2,0,columna,ACS_TTEE);

        for(int fila=1;fila<ALTO_CAJERO-1;fila++)
            mvwaddch(cajero2,fila,columna,ACS_VLINE);

        mvwaddch(cajero2,ALTO_CAJERO-1,columna,ACS_BTEE);
    }

    for(int i=0;i<PRODUCTOS;i++)
    {
        int inicio = 1 + i * (ANCHO_PRODUCTO + 1);

        string nombre = productos[i];

        mvwprintw(cajero2,
                  2,
                  inicio + (ANCHO_PRODUCTO - nombre.length())/2,
                  "%s",
                  nombre.c_str());
    }

    wrefresh(cajero2);

    refresh();
    getch();

    delwin(cajero1);
    delwin(cajero2);
}

int main()
{
    initscr();               // Iniciar ncurses
    noecho();                // No mostrar teclas
    cbreak();                // Entrada inmediata

	int yMax, xMax;
    getmaxyx(stdscr, yMax, xMax);
    box(stdscr, 0, 0);

    // TITULO
    string titulo[] ={
"  ____  _   _ ____  _____ ____  __  __ _____ ____   ____    _    ____   ___ ",
" / ___|| | | |  _ \\| ____|  _ \\|  \\/  | ____|  _ \\ / ___|  / \\  |  _ \\ / _ \\",
" \\___ \\| | | | |_) |  _| | |_) | |\\/| |  _| | |_) | |     / _ \\ | | | | | | |",
"  ___) | |_| |  __/| |___|  _ <| |  | | |___|  _ <| |___ / ___ \\| |_| | |_| |",
" |____/ \\___/|_|   |_____|_| \\_\\_|  |_|_____|_| \\_\\\\____/_/   \\_\\____/ \\___/ "
    };

    int altura_titulo = 5;

    attron(A_BOLD);

    for(int i = 0; i < altura_titulo; i++) {
        mvprintw(i + 1,(xMax - titulo[i].length()) / 2, titulo[i].c_str());
    }

    attroff(A_BOLD);

    //crear ventana
    WINDOW *menuwin = newwin(8, xMax - 10, altura_titulo + 2, 5);
    box(menuwin,0,0);
    refresh();
    wrefresh(menuwin);

    keypad(menuwin,true); //habilitar flechas

    string choices[3] =
    {
        "Iniciar",
		"Config",
        "Salir"
    };
	int choice;
    int highlight = 0;
    
    while(1)
    {
        box(menuwin,0,0);

        int ancho = getmaxx(menuwin);
        for(int i=0;i<3;i++)
        {
            int x = (ancho - choices[i].length())/2;
            if(i==highlight)
            wattron(menuwin,A_REVERSE);
            mvwprintw(menuwin,i+2,x,choices[i].c_str());
            wattroff(menuwin,A_REVERSE);
        }
        wrefresh(menuwin);
        choice = wgetch(menuwin);

        switch(choice)
        {
            case KEY_UP:
                highlight = (highlight - 1 + 3) % 3;
                break;
            case KEY_DOWN:
                highlight = (highlight + 1) % 3;
                break;
            default:
                break;
        }

        if(choice == 10)
        {
            switch(highlight){
                case 0:
				ventana_inicio();
				break;

				case 1:
				config_tiempos();
				break;

                case 2:
                endwin();
                return 0;
            }
        }
    }

    getch();
    endwin();

    return 0;
}